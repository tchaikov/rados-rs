...
use denc::{Denc, EntityAddr, EntityAddrType, EntityName, UTime};
...
fn bench_composite_message(c: &mut Criterion) {
    ...
    group.bench_function("encode", |b| {
        ...
        b.iter(|| {
            ...
            entity_name.encode(&mut buf, 0).unwrap();
            entity_addr
                .encode(&mut buf, denc::CephFeatures::MSG_ADDR2.bits())
                .unwrap();
    ...
    group.bench_function("decode", |b| {
        ...
        b.iter(|| {
            ...
            let _addr =
                EntityAddr::decode(&mut data, denc::CephFeatures::MSG_ADDR2.bits()).unwrap();
