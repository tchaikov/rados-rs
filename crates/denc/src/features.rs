...
mod tests {
    ...
    fn test_feature_constants() {
        assert_eq!(CEPH_FEATURE_PGID64, 1u64 << 9);
        assert_eq!(CEPH_FEATURE_PGPOOL3, 1u64 << 11);
        assert_eq!(CEPH_FEATURE_MSG_ADDR2, 1u64 << 59);
    }

    #[test]
    fn test_has_feature() {
        let features = CEPH_FEATURE_PGID64 | CEPH_FEATURE_PGPOOL3;
        assert!(has_feature(features, CEPH_FEATURE_PGID64));
        assert!(has_feature(features, CEPH_FEATURE_PGPOOL3));
        assert!(!has_feature(features, CEPH_FEATURE_OSDENC));
    }

    #[test]
    fn test_has_significant_feature() {
        let features = CEPH_FEATUREMASK_PGID64 | CEPH_FEATUREMASK_PGPOOL3;
        assert!(has_significant_feature(features, CEPH_FEATUREMASK_PGID64));
        assert!(!has_significant_feature(features, CEPH_FEATUREMASK_SERVER_LUMINOUS));
    }

    #[test]
    fn test_get_significant_features() {
        let all_features = u64::MAX;
        let sig_features = get_significant_features(all_features);
        assert_eq!(sig_features, SIGNIFICANT_FEATURES);
    }
}
