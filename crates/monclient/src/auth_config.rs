...
fn new_monitor_auth_provider(entity_name: &str) -> Result<auth::MonitorAuthProvider> {
    auth::MonitorAuthProvider::new(entity_name)
...
impl AuthConfig {
    ...
    pub fn from_ceph_conf(ceph_conf_path: &str) -> Result<Self> {
        ...
        let auth_provider = if auth_methods.contains(&auth::protocol::CEPH_AUTH_CEPHX) {
