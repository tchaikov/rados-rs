...
impl AuthConfig {
    ...
    pub fn from_ceph_conf(ceph_conf_path: &str) -> Result<Self> {
        let config = cephconfig::CephConfig::from_file(ceph_conf_path).map_err(|e| {
        ...
        let auth_provider = if auth_methods.contains(&CEPH_AUTH_CEPHX) {
            let keyring_path = config.keyring().map_err(|e| {
                MonClientError::ConfigError(format!("Failed to get keyring path: {}", e))
            })?;
            let mut provider = auth::MonitorAuthProvider::new(&entity_name).map_err(|e| {
                MonClientError::ConfigError(format!("Failed to create auth provider: {}", e))
            })?;
            provider
