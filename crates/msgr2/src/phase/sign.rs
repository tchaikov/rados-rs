    ...
    fn step(self, frame: Frame) -> Result<Step<Self, AuthSignClientOutput>> {
        match frame.preamble.tag {
            Tag::AuthSignature => {
                ...
                let peer_fs = crate::FeatureSet::from(self.peer_features);
                let our_fs = crate::FeatureSet::from(self.our_features);
                let needs_compression = peer_fs.contains(crate::FeatureSet::COMPRESSION)
                    && our_fs.contains(crate::FeatureSet::COMPRESSION);
