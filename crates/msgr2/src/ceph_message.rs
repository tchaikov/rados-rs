...
/// Implement [`CephMessagePayload`] for a [`denc::Denc`]-derived type.
///
...
macro_rules! impl_denc_ceph_message {
    ...
    (@internal $type:ty, $msg_type:expr, $version:expr, $($compat:expr)?) => {
        impl $crate::ceph_message::CephMessagePayload for $type {
            fn msg_type() -> u16 {
                $msg_type
            }
            ...
            fn encode_payload(
                ...
                let size = denc::Denc::encoded_size(self, features).unwrap_or(256);
                let mut buf = bytes::BytesMut::with_capacity(size);
                denc::Denc::encode(self, &mut buf, features)?;
                Ok(buf.freeze())
            }

            fn decode_payload(
                _header: &$crate::ceph_message::CephMsgHeader,
                front: &[u8],
                _middle: &[u8],
                _data: &[u8],
            ) -> std::result::Result<Self, $crate::Msgr2Error> {
                let mut data = front;
                Ok(denc::Denc::decode(&mut data, 0)?)
            }
        }
    };
}
