...
use crate::frames::{CRYPTO_BLOCK_SIZE, Frame, MessageFrame, Preamble, Tag, align_to_crypto_block};
...
impl FrameIO {
    ...
    pub async fn send_frame(
        ...
        let final_bytes = if !state_machine.has_encryption() {
            ...
            let mut preamble_block =
                BytesMut::with_capacity(crate::frames::PREAMBLE_SIZE + Self::INLINE_SIZE);
    ...
    pub async fn recv_frame(&mut self, state_machine: &mut StateMachine) -> Result<Frame> {
        ...
        let preamble_block_size = if has_encryption {
            crate::frames::PREAMBLE_SIZE + Self::INLINE_SIZE + Self::GCM_TAG_SIZE // 96 bytes
        } else {
            crate::frames::PREAMBLE_SIZE // 32 bytes
        };
        ...
        let preamble_bytes = preamble_and_inline.slice(..crate::frames::PREAMBLE_SIZE);
        let inline_data = if has_encryption {
            preamble_and_inline.slice(
                crate::frames::PREAMBLE_SIZE..crate::frames::PREAMBLE_SIZE + Self::INLINE_SIZE,
            )
        } else {
            Bytes::new()
        };
...
impl Connection {
    ...
    pub async fn send_keepalive(&mut self) -> Result<()> {
        ...
        let (timestamp_sec, timestamp_nsec) = crate::current_keepalive_timestamp()?;
