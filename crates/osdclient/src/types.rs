...
pub use denc::{SNAP_DIR, SNAP_HEAD};
...
impl OpResult {
    ...
    pub fn first_outdata(&self) -> crate::error::Result<&Bytes> {
        self.ops
            .first()
            .map(|op| &op.outdata)
            .ok_or_else(|| crate::error::OSDClientError::Other("No op result".into()))
    }
}
...
pub use crate::pg_nls_response::ListObjectImpl as ListObjectEntry;
