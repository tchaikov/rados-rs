...
use crate::messages::{HASH_CALCULATE_FROM_NAME, MOSDOp};
...
async fn await_op_result(
    rx: tokio::sync::oneshot::Receiver<Result<crate::types::OpResult>>,
...
impl OSDClient {
    ...
    fn parse_list_cursor(pool: u64, cursor: Option<String>) -> denc::HObject {
        match cursor {
            None => denc::HObject::empty_cursor(pool),
            Some(s) => {
                let hash: u32 = s.parse().unwrap_or(0);
                denc::HObject::new(pool, String::new(), hash)
            }
        }
    }
    ...
    async fn query_pg_objects(
    ...
    ) -> Result<(crate::PgNlsResponse, i32)> {
        ...
        let response = crate::PgNlsResponse::decode(&mut outdata.as_ref(), 0)?;
    ...
    fn build_list_result(entries: Vec<ListObjectEntry>, next_handle: &denc::HObject) -> ListResult {
    ...
    fn pg_hobj_end(pool: u64, pg_seed: u32, pg_num: u32) -> denc::HObject {
        ...
        if rev_end >= 0x1_0000_0000 {
            ...
            let mut h = denc::HObject::empty_cursor(pool);
            h.max = true;
            h
        } else {
            denc::HObject::new(pool, String::new(), (rev_end as u32).reverse_bits())
        }
    }
    ...
    async fn send_backoff_ack(
        ...
        use crate::messages::{CEPH_OSD_BACKOFF_OP_ACK_BLOCK, MOSDBackoff};
