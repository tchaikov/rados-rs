...
pub use frames::{
    DEFAULT_ALIGNMENT, EncodeWithFeatures, Frame, FrameAssembler, FrameTrait, MAX_NUM_SEGMENTS,
    MessageFrame, PREAMBLE_SIZE, Tag,
};
...
crate::define_options! {
    ...
    pub struct MessengerOptions {
        ...
        ms_dispatch_throttle_bytes: crate::cephconfig::Size = crate::cephconfig::Size(0),
    }
}
...
pub(crate) fn current_keepalive_timestamp() -> Result<(u32, u32)> {
    let now = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .map_err(|err| {
            Msgr2Error::invalid_data(&format!("system clock before UNIX_EPOCH: {err}"))
        })?;
    let timestamp_sec = u32::try_from(now.as_secs()).map_err(|e| {
        Msgr2Error::invalid_data(&format!(
            "system clock exceeds Keepalive2 timestamp range: {e}"
        ))
    })?;

    Ok((timestamp_sec, now.subsec_nanos()))
}

impl crate::Denc for FeatureSet {
...
pub struct AuthNonePayload {
    ...
    pub entity_name: crate::EntityName,
...
impl AuthNonePayload {
    ...
    pub fn for_monitor(entity_name: crate::EntityName, global_id: u64) -> Self {
        Self {
    ...
    pub fn for_service(entity_name: crate::EntityName, global_id: u64) -> Self {
        Self {
    ...
    pub fn encode(&self) -> Result<bytes::Bytes> {
        // auth_mode (u8=1) + EntityName (u8+u64=9) + global_id (u64=8) = 18 bytes
        let mut buf = bytes::BytesMut::with_capacity(18);
        use crate::Denc;
...
///
/// ```rust,no_run
/// use crate::msgr2::{ConnectionConfig, AuthMethod};
/// use crate::auth::MonitorAuthProvider;
///
...
pub struct ConnectionConfig {
    ...
    pub auth_provider: Option<Box<dyn crate::auth::AuthProvider>>,
...
impl Default for ConnectionConfig {
    fn default() -> Self {
        ...
        Self {
            ...
            entity_name: crate::EntityName::client("admin"),
...
impl ConnectionConfig {
    ...
    /// # Example
    /// ```no_run
    /// use crate::msgr2::ConnectionConfig;
    /// use crate::auth::MonitorAuthProvider;
    ///
    /// let mut mon_auth = MonitorAuthProvider::new("client.admin")?;
    /// mon_auth.set_secret_key_from_base64("AQA...")?;
    ...
    /// # Ok::<(), Box<dyn std::error::Error>>(())
    /// ```
    pub fn with_auth_provider(provider: Box<dyn crate::auth::AuthProvider>) -> Self {
    ...
    pub fn with_auth_provider_and_service(
        provider: Box<dyn crate::auth::AuthProvider>,
        service_id: u32,
    ) -> Self {
        Self {
