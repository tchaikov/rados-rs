...
impl Messenger {
    ...
    pub async fn start(&mut self) -> Result<()> {
        info!("Starting messenger {} on {}", self.local_name, self.local_addr);

        // Start listening for incoming connections
        let listener = TcpListener::bind(self.local_addr.addr).await
            .map_err(Error::from)?;
    ...
    pub async fn connect_to(&self, peer_addr: EntityAddr, peer_name: Option<EntityName>) -> Result<()> {
        info!("Connecting to {} ({})", peer_addr, peer_name.map(|n| n.to_string()).unwrap_or_else(|| "unknown".to_string()));
        ...
        let effective_peer_name = peer_name.or(peer_info.peer_name)
    ...
    pub async fn get_connection_info(&self, peer_name: EntityName) -> Option<crate::types::ConnectionInfo> {
        let connections = self.connections.read().await;
