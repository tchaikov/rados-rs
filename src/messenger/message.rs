...
impl Message {
    ...
    pub fn encode(&self, dst: &mut BytesMut) -> Result<()> {
        ...
        let total_len = MsgHeader::LENGTH + 
                       front_len as usize + 
                       middle_len as usize + 
                       data_len as usize;
...
impl fmt::Display for Message {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let type_name = match self.msg_type() {
            ...
            t => return write!(f, "Message(type={}, seq={}, len={})", t, self.seq(), self.total_len()),
        };
        
        write!(f, "Message({}[{}], seq={}, len={})", 
               type_name, self.msg_type(), self.seq(), self.total_len())
    }
}
...
impl MessageDispatcher {
    ...
    pub fn register_handler(&mut self, msg_type: u16, handler: Box<dyn MessageHandler>) {
        self.handlers.insert(msg_type, handler);
    }

    pub fn dispatch(&self, msg: Message, sender: EntityName) -> Result<Option<Message>> {
        if let Some(handler) = self.handlers.get(&msg.msg_type()) {
            handler.handle_message(msg, sender)
        } else {
            tracing::warn!("No handler registered for message type {}", msg.msg_type());
            Ok(None)
        }
    }
}
